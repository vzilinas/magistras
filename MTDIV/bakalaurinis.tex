...
\begin{enumerate}
    \item Sudarytas srautinio apdorojimo sistemų balansavimo modelį ir nustatytos valdymo metrikas ir jų siekiamas reikšmes, kurios naudojamos eksperimentinėje sistemoje.
...
\section{Srautinės duomenų apdorojimo sistemos, valdomos \mbox{skatinamuoju} mokymusi, modelis}
...
Reklamų analizės sistema rezultatus talpina į tekstinį failą, kur kiekvienas įrašas yra vėlinimas milisekundžių tikslumu nuo paskutinio išsiųsto įrašo į žinučių eilę tam tikram kampanijos langui iki kol jis yra įrašomas į Redis duomenų bazę. Pasinaudojant šiais duomenimis, po kiekvienos iteracijos apskaičiuojamas paskutinių 30 sekundžių vėlinimo vidurkis. 

Eksperimentai atlikti su visais algoritmais apmokius juos 50 iteracijų, 100 iteracijų, 250 iteracijų ir 1000 iteracijų tuo siekiant palyginti rezultatus tarp algoritmų ir mokymosi trukmės. Algoritmo efektyvumui patikrinti apmokytas modelis atlieka konfigūracijos keitimą 25 iteracijas ir matuojamas vėlinamas po kiekvieno pakeitimo praėjus 3 minutėms – laikas, kurio reikia srautinio apdorojimo sistemai pasileisti ir pasiekti stabilią apkrovos stadiją. Kadangi srautinio apdorojimo sistemos nėra pastovios, rezultatų įvertinimui apsibrėžta paklaida – 5 sekundės vėlinimo.

\subsection{Eksperimentai}
Eksperimentų tikslas – išmatuoti srautinės apdorojimo sistemos Reklamų analizės vėlinimą balansuojant ją naudojant atitinkamą algoritmą, palyginti vėlinimą su skirtingu kiekiu apmokymo žingsnių ir palyginti algoritmų rezultatus.

Tinklai yra apmokyti 4 atskirus kartus su 50, 100, 250 ir 1000 žingsnių. Po apmokymo su skirtingu žingsnių kiekiu sistema paleidžiama iš naujo ir įvykdomi papildomi 25 testavimo žingsniai, kurių metu tinklas nėra mokomas, o renkami rezultatai. Surinkti rezultatai, pateikti grafikuose, rodo srautinės apdorojimo sistemos vėlinimą keičiant konfigūraciją naudojant atitinkamą algoritmą.

\subsubsection{Eksperimentas nr. 1 – balansavimas naudojant REINFORCE tinklą}
Skirtingo apmokymo kiekio rezultatai:
\begin{itemize}
...
\begin{figure}[H]
    ...
    \end{minipage}
    \caption{Balansavimo su REINFORCE algoritmu rezultatai}
    \label{reinforce–rez}
\end{figure}
REINFORCE algoritmas su visais išbandytais apmokymo kiekiais testavimo metu pasiekia mažesnį vėlinimą. Visų bandymu metu sumažėjimas yra labai panašus – 15–20\%. Apmokius 50, 100, 250 žingsnių algoritmas pirmais testavimo žingsniais vėlinimas svyruoja, o apmokius 1000 žingsnių svyravimas dingsta. Visais apmokymo kiekiais apmokyto REINFORCE tinklo vėlinimo rezultatai po 5 žingsnių keičiasi nežymiai.

Kadangi algoritmas su visais apmokymo kiekiais pasiekia labai vienoda sumažinimą, galima teigti, jog REINFORCE tinklas balansuoja srautinio apdorojimo sistemą nedideliais žingsniais ir pasiekia patikimus rezultatus. Tai reiškia, kad tokio tipo algoritmas išlaiko sistemą stabilią ir pasiekia vėlinimo mažinimą. Taip pat, kadangi vėlinimo rezultatai vienodi, galima teigti, jog šis algoritmas reikalauja nedaug apmokymo žingsnių.
...
\begin{itemize}
    ...
    \item 1000 žingsnių – testavimo metu vėlinimas išauga nuo 115 sekundžių iki 144 sekundžių.
\end{itemize}
\begin{figure}[H]
    \centering
    \begin{minipage}[b]{0.45\textwidth}
    ...
    \end{minipage}
    \caption{Balansavimo su Deep Q Network algoritmu rezultatai}
\end{figure}
Deep Q Network algoritmas su 50, 100 ir 250 apmokymo žingsnių vėlinimą sumažina, tačiau nežymiai. 100 ir 250 apmokymo žingsnių tinklas turi daug vėlinimo svyravimų, o 50 ir 1000 neturi svyravimų. 1000 žingsnių apmokytas algoritmas pabaigoje turi staigų augimą, nuo 100 sekundžių iki 140 sekundžių vėlinimo.

Su visais apmokymo žingsnių kiekiais algoritmas pasiekia labai nedidelį pokyti, kas leidžia teigti, jog algoritmui nepakankamai mokymosi žingsnių, kad būtų atliktas išsamus tyrinėjimas ir algoritmas renkasi saugius veiksmus, kad dažniau būtų gaunamas neneigiamas atlygis. 1000 žingsnių apmokytas tinklas pasiekia mažesnį vėlinimą iki 20 žingsnio (nuo 115 iki 92 sekundžių), tačiau po šio žingsnio atliekamas veiksmas pakelia vėlinimą per 40 sekundžių ir todėl stipriai nukenčia bendras DQN algoritmo rezultatas.

\subsubsection{Eksperimentas nr. 3 – balansavimas naudojant Actor–Critic with Experience Replay tinklą}
Skirtingo apmokymo kiekio rezultatai:
\begin{itemize}
    \item 50 žingsnių – testavimo metu vėlinimas išauga nuo 151 sekundžių iki 171 sekundžių. 
    ...
    \item 1000 žingsnių – testavimo metu vėlinimas sumažėja nuo 140 sekundžių iki 68 sekundžių.
\end{itemize}
\begin{figure}[H]
    \centering
    \begin{minipage}[b]{0.45\textwidth}
    ...
    \end{minipage}
    \caption{Balansavimo su Actor–Critic with Experience Replay algoritmu rezultatai}
\end{figure}
Actor–Critic with Experience Replay apmokytas 1000 žingsnių sumažina vėlinimą 50 procentų ir neturi žymių svyravimų. Apmokytas 50 žingsnių algoritmas po 5 žingsnių sumažina vėlinimą nuo 151 iki 85 sekundžių, tačiau po to vėlinimas kyla ir pasiekia didesnį negu buvo pradiniame žingsnyje. Panašu sumažinimą kaip 50 žingsnių apmokymą turi ir 250 žingsnių apmokytas tinklas, kuris taip pat testavimą baigia su didesniu vėlinimu negu pradiniame žingsnyje. 100 žingsnių apmokytas tinklas pasiekia sumažinta vėlinimą, tačiau viso testavimo metu vėlinimas svyruoja daugiau nei paklaida.

Actor–Critic with Experience Replay pasirodo labai gerai, kai yra apmokytas 1000 žingsnių ir pagal gautus rezultatus, galima tikėtis, jog su didesniu kiekiu apmokymo, turėtų galėti pasiekti dar geresnius rezultatus. Tačiau su mažiau nei 1000 žingsnių rezultatai yra labai stipriai svyruojantys – 50 ir 250 rezultatai rodo didelius sumažėjimu ir taip pat vėlinimo didėjimus. Galima teigti, jog 50 ir 100 apmokytas tinklas daro labiau spėjimus negu sprendimus. O 250 žingsnių apmokytas tinklas yra išmokęs gerus sprendimus iki 10 žingsnio, bet nėra pakankamai apmokytas mažinti vėlinimą viso testavimo metu.
...
\subsection*{Išvados}
Kadangi REINFORCE algoritmo balansuojamų srautinių sistemų vėlinimas yra sumažinamas 15-20\% ir apmokius skirtingais kiekiais žingsnių balansavimas nesukuria vėlinimo svyravimų didesnių nei 5 sekundės, šis algoritmas yra tinkamas balansuoti srautines apdorojimo sistemas, kurios turi išlikti stabilios veikimo metu.

Kadangi Deep Q Network pasiekia stipriai svyruojančius rezultatus, nuo 20 sekundžių sumažinto vėlinimo iki beveik 30 sekundžių padidinto vėlinimo galima teigti, jog šio algoritmo tinkamumas balansuoti srautinio apdorojimo sistemas nėra įrodytas šiame darbe ir, kad būtų tinkamas gali reikalauti daugiau apmokymo žingsnių.

Kadangi Actor–Critic with Experience Replay algoritmo balansuojamų srautinio apdorojimo sistemų vėlinimas yra sumažinamas iki 50\%, šis algoritmas yra tinkamas balansuoti srautinio apdorojimo sistemos, kurios reikalauja žemo vėlinimo. Tačiau šio algoritmo balansavimo rezultatai priklauso nuo mokymo žingsnių ir algoritmas pasiekia mažesnį vėlinimą nei pradiniame žingsnyje tik po 1000 žingsnių apmokymo.  

Remiantis šiomis išvadomis galima teigti, jog srautinio apdorojimo sistemas galima balansuoti naudojant skatinamąjį mokymąsi, tačiau skatinamojo mokymosi algoritmas ir apmokymo kiekis turi būti pasirinktas ir pritaikytas srautinio apdorojimo sistemai.
