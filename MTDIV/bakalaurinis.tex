...
\begin{enumerate}
    ...
    \item Palyginti eksperimento rezultatus su alternatyvomis – „Heron“ su standartine konfigūracija (??? ar reikia ->) bei „Heron“ balansavimas rankiniu būdu. 
\end{enumerate}
...
\subsection{Skatinamasis mokymasis srautinio apdorojimo veikimo gerinimui}
...
Šiame darbe norint patikrinti ar skatinamasis mokymasis tinka srautinio apdorojimo sistemų balansavimui naudojami keli algoritmai siekiant užtikrinti, jog rezultatai atitiktų skatinamojo mašininio mokymosi gebėjimą balansuoti, o ne pavienio algoritmo. Pasirinkti algoritmai: 
\begin{itemize}
    ...
    \item Actor-Critic with Experience Replay remiantis \cite{wang2016sample}, (-> hmm???) kadangi jam reikia mažiau duomenų nei Deep Q Network .
\end{itemize}
...
\begin{longtable}{|p{0.1\linewidth}|p{0.6\linewidth}|p{0.2\linewidth}|}
...
\begin{itemize}
 ...
 \item Atlygis – skaitinė reikšmė apskaičiuotą naudojant normalizuotas metrikas (apibrėžtas \ref{metrics–table} pav.) su koeficientais ir \ref{formula} formulę.
\end{itemize}
\begin{equation}
    \label{formula}
    \begin{gathered}
    \text{Atlygis} = 2*\text{Išsiųstas kiekis } + 2*\text{Apdorotas kiekis }  – \\ 
...
Reklamų analizės sistema rezultatus talpina tekstini failą, kur kiekvienas įrašas yra vėlinimas milisekundėmis nuo paskutinio išsiųsto įrašo į žinučių eilę tam specifiniam kampanijos langui iki kol jis yra įrašomas į Redis duomenų bazę. 
